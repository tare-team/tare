...
def dev(
    sync: bool = typer.Option(
        False, help="Run 'uv sync' before starting services"
    ),
) -> None:
    """Start the local development environment.

    This command boots supporting services via Docker, optionally synchronizes
    Python dependencies, then starts the GraphQL API server and the React
    development server. Processes run until interrupted with CTRL+C.
    """
    ...
    if sync:
        typer.echo("Syncing Python packages...")
